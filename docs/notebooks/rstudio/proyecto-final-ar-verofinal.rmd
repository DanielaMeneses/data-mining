...
```{r}
NOMBRE_COMPLETO = "Verónica Hernández"
DIGITOS_MATRICULA = 01745248
```
...
- ¿Cuál es la interpretación de la regla `{} => {diapers}` en la que se aprecia la ausencia del LHS? (**NOTE-19**):En la primera regla se muestra que solo compraron pañales, es decir, no tiene una relación con otros productos. 
...
- ¿Por qué no aparece listada la regla `{beer} => {nuts}`? (**NOTE-22**): En este caso, las metricas de confidence y support excluyen la regla antes mencionada. 
...
- ¿Cuál es el propósito de `session id`? (**NOTE-23**):unique session identifier, en este caso, cada usurio tiene su propio id de sesion con el objetivo de que a cada persona le pueda recomendar canciones de acuerdo a sus preferencias.  
- ¿Cuál es el propósito de `track id`? (**NOTE-24**):unique track identifier, Aqui se tiene que cada canción debe de tener su propio ID con el objetivo de contar cuantas veces se reprodujo y que otras canciones se pueden relacionar con  esta para poder realizar recomendaciones. 
- ¿Consideras que las otras columnas del dataset son aplicables para análisis por medio de reglas de asociación, justifica tu respuesta? (**NOTE-25**): Si, pues como se observó en clase las reglas nos ayudan a relacionar sucesos, en este caso,Spotify lo ocupa para realizar recomendaciones pues ve como es que el usuario interactua con las diferentes herramientas, ademas de que nos ayuda a ver las tendencias que se tienen. 
...
t_bacf06d3-9185-4183-84ea-ff0db51475ce con 886

- ¿Cuántos items tiene el itemset con mayor número de ocurrencias en la lista element length distribution? (**NOTE-27**):
El item 20 tiene una ocurrencia de 1593 
- En el contexto de este dataset con información de sesiones de streaming, ¿cuál es la interpretación de la media aritmética? (**NOTE-28**): Los usuarios ingresan a la sesión al menos 14.61 veces en promedio.
...
En este caso que no se produce ninguna regla es porque ningun conjunto de items cumplen con los parametros que se le asignaron,
- ¿Qué riesgos identificas con la asignación de valores muy bajos en los parámetros de  `support` y `confidence` al invocar el algoritmo `apriori`? En este caso, se va a tardar mas el algoritmo en ejecutar. Además, puede perder precisión pues como son valores muy pequeños ya no aparecen en el análisis. 
